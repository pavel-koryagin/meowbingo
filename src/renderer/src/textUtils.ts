...
const notLetterSpaceHyphenRegex =
...
export function getWords(sentence: string): string[] {
  return sentence
    .replace(notLetterSpaceHyphenRegex, '')
    .split(' ')
    .map((v) => _trim(v, '–-'))
    .filter((v) => v)
}
...
export function getQualifiedWords(expectedAnswer: string, actualAnswer: string): QualifiedWord[] {
  ...
  return words.map((withPunctuation) => {
    const word = withPunctuation.replace(notLetterSpaceHyphenRegex, '').trim()

    return {
      withPunctuation,
      word,
      matchesTyped: lcTypedWords.includes(word.toLowerCase())
    }
  })
}
