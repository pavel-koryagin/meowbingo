...
const AUDIO_PATH = 'audio' // In userData

const REGEX_CHAR_TO_ESCAPE =
...
function escapeAudioFilename(filename) {
  return filename
    .trim()
    .replace(/\s+/g, ' ')
    .replace(/\.$/, '')
...
export async function getPronunciation(sentence: string): Promise<string | null> {
  ...
  const file =
    settings.userDataPath + '/' + AUDIO_PATH + '/' + escapeAudioFilename(sentence) + '.mp3'

  if (fs.existsSync(file)) {
    return makeLink(await fs.promises.readFile(file))
  }

  // Check for play.ht subscription
  if (!settings.playHtUserId) {
    // console.log(`Lacking audio for\n${sentence}\n${escapeAudioFilename(sentence)}`)
    return null
  }

  // Note: Handle exceptions on the frontend
...
export async function requestPlayHtGeorgianTTS(
  ...
  const { data } = await axios.post('https://play.ht/api/transcribe', {
    ...
    ssml: `<speak><p>${_escape(sentence)}</p></speak>`,
