...
function escapeAudioFilename(filename) {
  return filename.replace(REGEX_CHAR_TO_ESCAPE, (char) => encodeURIComponent(char))
}
...
export async function getPronunciation(sentence: string): Promise<string | null> {
  ...
  const file =
    settings.paths.userData + '/' + AUDIO_PATH + '/' + escapeAudioFilename(sentence) + '.mp3'
  ...
  const audioData = await requestPlayHtGeorgianTTS(sentence)
...
export async function requestPlayHtGeorgianTTS(sentence): Promise<ArrayBuffer> {
